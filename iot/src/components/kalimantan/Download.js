...
const Download = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch both stations simultaneously
        const [res1, res2] = await Promise.all([
          fetch(API_KALIMANTAN_TOPIC1),
          fetch(API_KALIMANTAN_TOPIC2)
        ]);
        
        console.log("Response status:", { 
          station1: res1.status, 
          station2: res2.status,
          station1_ok: res1.ok,
          station2_ok: res2.ok 
        });
        ...
        if (!res2.ok) {
          ...
          console.error("Station 2 API error:", text2.substring(0, 200));
        }
        
        const [json1, json2] = await Promise.all([
          res1.ok ? res1.json() : { result: [] },
          res2.ok ? res2.json() : { result: [] }
        ]);
        
        console.log("Station 1 API response:", json1);
        console.log("Station 2 API response:", json2);
        ...
        console.log("Mapped Station 2 data:", station2Data.length, station2Data.slice(0, 3));
  ...
  useEffect(() => {
    ...
    console.log("Station 2 Data updated:", station2Data.length);
    console.log("Data ready:", dataReady);
  }, [station1Data, station2Data, dataReady]);
  ...
  const processDownload = () => {
    ...
    if (enableResampling) {
      ...
      try {
        // Tentukan fields yang akan di-resample berdasarkan station
        let fields = ['humidity', 'temperature'];
        if (selectedStation === 'Station 1') {
          fields = [...fields, 'rainfall', 'windspeed', 'irradiation', 'windDirection', 'angle'];
        }
    ...
    const baseFilename = `${selectedStation.replace(' ', '_')}_data${interpolationSuffix}${smoothingSuffix}${resampleSuffix}`;
          ...
          {!loading && dataReady && (
            <div className="mb-3">
              ...
              <small className="text-muted">
                Station 1: {station1Data.length} records | Station 2: {station2Data.length} records
                <br />
                {enableInterpolation && (
